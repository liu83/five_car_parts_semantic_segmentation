...
Background removal in the AUTO1 dataset means each image has a large
...
IMPORTANT: this crop is derived from the IMAGE content only (detecting the
background color), never from the mask. Test images have no mask, so the
crop logic must work identically for train and inference — deriving it
from the image guarantees that.
"""

import numpy as np


def estimate_background_color(image: np.ndarray, border_frac: float = 0.02) -> np.ndarray:
...
def compute_foreground_bbox(image: np.ndarray, bg_color: np.ndarray,
                             threshold: float = 20.0, margin_frac: float = 0.03):
    ...
    """
    diff = np.abs(image.astype(np.float32) - bg_color.astype(np.float32)).sum(axis=2)
...
def crop_to_object(image: np.ndarray, mask: np.ndarray = None,
                    threshold: float = 20.0, margin_frac: float = 0.03):
