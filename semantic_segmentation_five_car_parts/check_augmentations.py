"""
check_augmentations.py — visualize augmented (image, mask) pairs so you can
eyeball two things before a full training run:

    1. Alignment: does the mask still line up with the image after
       geometric transforms (flip/rotate/scale)? A bug here (e.g. using
       torchvision transforms separately on image and mask) would show the
       colored part overlay drifting away from the actual part in the image.
    2. Strength: are the augmentations reasonable, or too aggressive
       (e.g. rotation/brightness so extreme the part is barely recognizable)?
...
Usage:
    python3 check_augmentations.py --images_dir data/images --masks_dir data/masks \
...
def overlay_mask(image: np.ndarray, mask_tensor) -> np.ndarray:
    ...
    for class_idx, color in CLASS_COLORS.items():
        ...
        result[class_pixels] = (
            result[class_pixels] * (1 - OVERLAY_ALPHA) + color_arr * OVERLAY_ALPHA
        )
...
def main():
    parser = argparse.ArgumentParser(description="Visualize augmented image/mask pairs")
    ...
    pairs = [(p, masks_dir / f"{p.stem}.png") for p in all_images
             if (masks_dir / f"{p.stem}.png").exists()]
    if not pairs:
        raise FileNotFoundError(f"No matching image/mask pairs found under {images_dir} / {masks_dir}")

    rng = random.Random(args.seed)
    sample = rng.sample(pairs, min(args.num_images, len(pairs)))

    crop_to_bbox = not args.no_bbox_crop
    reference_transform = get_transforms(train=False, img_size=args.img_size,
                                          pad_to_square=args.pad_to_square)
    train_transform = get_transforms(train=True, img_size=args.img_size,
                                      pad_to_square=args.pad_to_square)

    reference_ds = CarPartsDataset([p for p, _ in sample], [m for _, m in sample],
                                    reference_transform, crop_to_bbox=crop_to_bbox,
                                    bbox_threshold=args.bbox_threshold, bbox_margin=args.bbox_margin)
    train_ds = CarPartsDataset([p for p, _ in sample], [m for _, m in sample],
                                train_transform, crop_to_bbox=crop_to_bbox,
                                bbox_threshold=args.bbox_threshold, bbox_margin=args.bbox_margin)

    print(f"Visualizing {len(sample)} images x {args.num_augs} augmented variants each\n")
    ...
    for i, (img_path, _) in enumerate(sample):
        ...
        for a in range(args.num_augs):
            aug_image_t, aug_mask_t = train_ds[i]  # re-sampling triggers new random augmentation
        ...
        final = Image.new("RGB", (row.width, row.height + legend.height), color=(255, 255, 255))
        final.paste(row, (0, 0))
    ...
    print("Check that the colored overlay stays aligned with the actual part "
